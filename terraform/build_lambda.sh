...
#
# 今は ingest/api/watchdog/detect の4本（rollup は未実装 → docs/roadmap.md）。
...
build_api() {
  ...
  # /events が grid_events.recent_events (detectイベント台帳の読み取り) を使うので同梱する。
  cp -r "$LAMBDA/common" "$stage/common"
...
build_detect() {
  stage="$BUILD/detect"
  mkdir -p "$stage"
  cp "$LAMBDA/detect/handler.py" "$stage/handler.py"
  cp "$LAMBDA/s3keys.py" "$stage/s3keys.py"
  cp "$LAMBDA/wire_gridfreq.py" "$stage/wire_gridfreq.py"
  cp "$LAMBDA/store_gridfreq.py" "$stage/store_gridfreq.py"
  # grid_detect(しきい値判定)・grid_events(DynamoDB台帳)。→ docs/cloud.md「detect」
  cp -r "$LAMBDA/common" "$stage/common"
  # Slack通知(notify.from_env)を使うのでbatch_uplinkを同梱する。
  "$PY" -m pip install --quiet --no-deps \
    --target "$stage" "git+https://github.com/nna774/batch-uplink@$UPLINK_VERSION" >/dev/null
  find "$stage" -name '__pycache__' -type d -prune -exec rm -rf {} +
  (cd "$stage" && zip -qr "$BUILD/detect.zip" .)
  echo "built $BUILD/detect.zip"
}
