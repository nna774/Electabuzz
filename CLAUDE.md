...
**detect(周波数逸脱・RoCoF・電圧異常の確定判定、フェーズ9)は`apply`・実クラウド
起動確認まで完了した**（2026-08-17、`lambda/detect/`・`lambda/common/grid_detect.py`
(純粋関数)・`lambda/common/grid_events.py`(DynamoDBセッション管理)・`/events` API・
...
- **デプロイ作業中に新たな問題を見つけても、追加の修正→PRマージ→再デプロイまで
  独断で進めるな。** 「デプロイして」のような指示は、その時点で提示・合意済みの
  変更を配ることの許可であって、作業中に新たに見つかった別件の修正までは含まない。
  新たに生じた変更は都度報告し、マージ・デプロイの可否を確認しろ(一度の許可は
  次のPRには引き継がない。force pushの許可と同じ精神)。実機1台が本番データを
  送り続け、ダッシュボードも公開済みで、デプロイは実コストと稼働中システムに
  直接効く。2026-08-27、`/recent`並行化のデプロイ中にS3接続プール不足を見つけ、
  その場で修正・PR化・マージ・再デプロイまで独断で行ってしまい訂正された
  （デプロイ自体は事前の指示どおりで問題なかった）
...
2. **[docs/progress.md](docs/progress.md) の表に1行追記する**（1〜3文の要約 + ログへのリンク。
   並行セッションのコンフリクトを避けるため`.gitattributes`で`merge=union`を設定してある——
   厳密な日付順が崩れることがあるが気にしなくてよい）。
