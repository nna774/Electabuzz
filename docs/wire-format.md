...
| u16 | `v_rms_mv` — **トランス二次側**の実効値[mV]。電圧異常・停電判定用 |
...
**`v_rms_mv` の基準点は決定済み: トランス二次側の実効値[mV]。**(2026-08-13)
商用の 100V を mV で持つと u16(最大 65.535V)に収まらないが、二次側(無負荷10.5V前後)
なら`10500`程度で収まり余裕も大きい。**壁側(商用100V系)への換算は持たない。**
用途(電圧異常・停電判定)は同一デバイス内の相対的な落ち込みが見えれば足り、
分圧比(R1/R2、固定の設計値)だけで完結し、個体差のあるトランス巻数比という
追加の校正定数を持ち込まずに済む。壁側の値が要る場面が来たら、**巻数比の定数を
別途 [hardware.md](hardware.md) に置き、ダッシュボード/Lambda側で `secondary_mv × 巻数比`
として計算する**——ワイヤフォーマット自体の変更は不要。→
[open-questions.md](open-questions.md)、
[log/2026-08-13-vrms-basis-point-decision.md](log/2026-08-13-vrms-basis-point-decision.md)
