...
| 日付 | 何が決まったか | 詳細 |
|---|---|---|
| 2026-08-08 | **NOMINAL区間（起動直後〜NTPロックまで約10分）の対処方針を「案A」に決定した。** `tools/gridfreq/goertzel.py`に既知のppm誤差(-1000〜+1000ppm)を注入して系統誤差を測ったところ、線形回帰の決定係数R²=0.99999939、実際の水晶誤差の10倍以上(±1000ppm)まで振っても線形予測からの残差は最大0.049mHz(目標精度±1mHzの1/20以下)——**「fs誤差はスカラー補正で戻る」という前提を検証できた。** これにより、NOMINALで即記録・即送信しNTPロック後に補正レコードを別送する案A(生サンプル保持が必要な案は容量的に不成立、window単位で一時保持し送信を遅らせる案Bより即時性で優る)を採用。ダッシュボードはNOMINAL区間をそれと分かる線で描き、補正到着後に同区間へ「補正した予測値」の線を重ねる。**まだ未実装**（ワイヤフォーマット・ストア・ダッシュボードの3箇所に手を入れる必要がある） | [log/2026-08-08-nominal-window-open-question.md](log/2026-08-08-nominal-window-open-question.md) |
