...
| | |
|---|---|
| 確定済み | AC入力部（実測済み）、wire format `GFRQ` v1、**[batch-uplink](https://github.com/nna774/batch-uplink) v1.6.0**（public・切り出し済み。`Batch` の契約が確定。pinはNamazu側の後方互換な追加でv1.0.0から上げてある → [log/2026-08-07-goertzel-cpp-port.md](log/2026-08-07-goertzel-cpp-port.md)） |
...
- **NOMINAL区間(起動直後〜NTPロックまで)の補正記録を実装する**（2026-08-08に方針決定。
  ①補正レコードのワイヤフォーマット(対象カウント区間の特定方法、εppmフィールド)
  ②`store_gridfreq.py`側の補正適用ロジック ③ダッシュボードの2本線描画(NOMINAL区間の線
  + 補正到着後の予測値の重ね描き)。→ [log/2026-08-08-nominal-window-open-question.md](log/2026-08-08-nominal-window-open-question.md)）
