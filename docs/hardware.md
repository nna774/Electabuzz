...
- **分圧比は 1/14 を出発点とする。** PCM1808 のフルスケール入力は
  **3 Vp-p @ VCC = 5V**（= 0.6 VCC、データシート値）。二次側の実測 29.6 Vpp に対し
  フルスケールの **70%**（= 2.1 Vpp）を狙うと `2.1 / 29.6 ≒ 1/14`。
  例: R1 = 130kΩ / R2 = 10kΩ。**これは机上計算であって実測ではない。**
  DMM が届いたら壁電圧と二次側を測って詰め直す。
...
**電源は2系統要る。** `VCC`（アナログ）は **4.5〜5.5V** で、モジュールにレギュレータは
載っていない。**3.3V だけ与えても動かない。** `VDD`（デジタル）が 2.7〜3.6V。
