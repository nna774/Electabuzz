...
ESP32-S3 + GNSS で 1W 程度なので 10000mAh で数十時間。

> **AC入力断の検知方式は「v_rms低下」から変わっている(2026-08-12)。**
> 実機でAC入力線がブレッドボードから抜ける事故を受けて回路解析したところ、
> 現行AFE(C1実装後)では入力断時にnode AがR2経由で低インピーダンスにGNDへ
> 落ちて振幅ゼロに静止する素直な壊れ方をすると判明。検知は**`v_rms_mv`(=Goertzel
> ビンのエネルギーから換算した値)がしきい値未満で継続すること**で行う。
> **そして「線が抜けた」と「停電した」はAFE単体の信号では原理的に区別できないと
> 分かった**——UPS化後はESP32が電池で生き続けAFEだけが入力ゼロを見る状態になり、
> 両者は同じ見え方になる。**方針は無理に区別せず「AC入力が見えない」で一本化、
> 原因の切り分けは通知を受けた人間が現地で判断する**、に決めた。
>
> **2026-08-15、検知ロジック・LED通知・`flags`配線を実装した。**
> `firmware/lib/AcInputMonitor/`(しきい値`kAcFaultVRmsThresholdMv=1000mV`未満が
> `kAcFaultSustainWindows=3`window連続でfault確定、対称なヒステリシスで復帰も同数を
> 要求)が判定し、`firmware/lib/FaultNotify/`の`FaultNotifier`インターフェース経由で
...
> [docs/log/2026-08-15-ac-input-disconnect-detection-impl.md](log/2026-08-15-ac-input-disconnect-detection-impl.md)、
> [open-questions.md](open-questions.md)「AC入力線の入力断検知・物理固定」）

> **「小型UPSを挟む」の具体案: 電源マルチプレクサIC(TPS2113A系)で
> USBパススルー+乾電池バックアップを自動切替し、ICのSTAT出力をそのままGPIOの
> フラグに使う。** 市販パススルーバッテリーと違い機械可読な
> 「今どちらの電源か」フラグを追加回路無しで得られる点が利点。
>
> **部品・配線・容量計算は2026-08-16に具体化した**（→
> [docs/log/2026-08-16-ups-power-mux-parts-selection.md](log/2026-08-16-ups-power-mux-parts-selection.md)、
> 発注・実装はまだ）。
>
> - **マルチプレクサ本体**: [Pololu-2596](https://www.switch-science.com/products/7017)
>   (TPS2113Aキャリア、¥1,793)。`STAT`/`EN`/`ILIM`/`VSNS`も引き出されている
> - **配線は`OUT`→ESPの`5V`ピン、`3V3`ピンには絶対に繋ぐな。** `OUT`はレギュレータ
>   ではなくパススルーで、このボードの`micro-B USBコネクタはデフォルトで`IN1`に
>   直結される(自動切替はIN1/IN2の高い方を選ぶ)ため、USB接続中`OUT`は生の5V近辺に
>   なる。`3V3`ピン(絶対最大定格3.6V程度)に直結すればUSB接続中に壊れる
> - **昇圧(昇降圧)コンバータは省略できない、と結論が変わった。** 上記配線を採る以上
>   IN2(電池側)もESPのオンボードレギュレータが3.3Vを安定して出せる電圧
>   (概ね4.3〜4.6V以上)を用意する必要があり、単3数本の直列だけでは放電で割り込む。
>   **採用: [Pololu S9V11F5](https://www.switch-science.com/products/3246/)**
>   (¥2,332、入力2〜16V・出力5V固定・Vin≈5V付近でtyp 1.5A・**`Vin<7V`でIq typ <100µA**・
>   効率90%超)。単3×4本(新品6.4V〜末期4V)は丸ごとIq<100µA帯に収まり、
>   出力電流も想定ピーク(0.5A)に対し余裕がある。無負荷Iqが低く出力電流も稼げる
>   物件を選んだことで、「昇圧の常時通電による待機中の乾電池消耗」は解消した
> - **出力バルクコンデンサはオーダーで100〜220µF。** TPS2112A/2113Aデータシートの
>   Transition time(`IN1`→`IN2`の自動切替、typ 40µs/max 60µs @ `I_L=500mA`)から
>   `ΔV=I・Δt/C`で算出(`ΔV=0.3V`は仮置き)。実機でのスコープ確認は未実施
> - **電池種別はアルカリよりリチウム一次電池(Energizer Ultimate Lithium級)を推す。**
>   母艦消費電力の見積り(1W級、未実測)ベースで単3×4本の稼働時間を試算すると、
>   アルカリ約8〜11時間・リチウム一次電池約15時間。保存性だけでなく稼働時間でも勝る
> **ただしこのSTATが表すのは「母艦(ESP32)自身がどちらの電源経路から給電されているか」
> であり、AFEの入力経路(トランス→AFE)とは物理的に別系統。** 両者は必ずしも連動しない
> ——例えばUSB配線を整理する際にUSBだけ一時的に抜けばSTATは電池側に切り替わるが
> トランス側は生きたままで、AFEの振幅は正常に出続ける。**STATを記録するとしても
> 「AC入力が見えない」問題を自動判別する根拠には使わない**——`timebase_source`と
> 同じ立て付けで独立した品質ビットとして記録するに留め、原因の切り分けは
> 引き続き人間に委ねる(上のAC入力断の項と同じ判断)。

### `v_rms_mv` の基準点とトランス巻数比 (2026-08-13 決定)

`GfrqRecord.v_rms_mv`は**トランス二次側の実効値[mV]**を記録する(→
[wire-format.md](wire-format.md))。壁側(商用100V系)への換算は持たない——
用途(電圧異常・停電判定)は同一デバイス内の相対的な落ち込みが見えれば足り、
分圧比(R1/R2、固定の設計値)だけで完結する。個体差のあるトランス巻数比という
追加の校正定数をワイヤフォーマットに持ち込まずに済む。

**壁側電圧が欲しくなったら、ここにある巻数比の定数を使って downstream
(ダッシュボード/Lambda側)で `secondary_mv × 巻数比` を計算すればよい。
ワイヤフォーマットの変更は不要。**

> **巻数比は暫定値: ≈10.08倍(2026-08-15実測、複数回で確認中)。** DMM2台で壁側・二次側を
> 同時に2回実測した: 1回目 102.8V / 10.21V → 10.07倍、2回目 102.9V / 10.21V → 10.08倍。
> 直前に同一DMM1台で数秒ずらして測った参考値(102.9V / 10.19V → 10.10倍)も合わせて
> 3回とも比で0.3%以内に収まっており、この時間スケールでは壁電圧の変動が無視できる
> ことも確認できた。旧暫定値 9.71倍(2026-08-03算出、「現物検証ログ」本ファイル135行目の
> DMM実測10.3VACを100/10.3で近似)からの約4%のズレは、当時壁側電圧を実測せず
> 100V決め打ちで計算していたことに起因する(実際の壁は102.8〜102.9V付近だった)。
> **複数回・複数日にわたる再測定で安定性を見てから確定値へ格上げする予定**
> (日本のコンセントは実際には101〜107V程度でばらつくため)。

> **`v_rms_mv`自体もAFE換算式の経験校正が入った(2026-08-15)。** 上記の
> DMM実測(二次側10.21V)とほぼ同時刻帯のfirmware生値(約8480mV)を比べたところ
> 約1.204倍の系統的な食い違いがあり、`R1`/`R2`公称値・ADC入力インピーダンス
> データシートtyp.値からの理論式だけでは実測を再現できないと判明した。
> node A(PCM1808のL IN、分圧後)をDMMで直接実測(0.577V)してみたところ、
> 二次側実測×理論分圧比とのズレは1.8%(抵抗誤差の範囲)で**分圧網は無罪**、
> VCCも再実測(4.83V、旧実測4.84Vと同じ)で**無罪**と確定できた。ズレの正体は
> ADCコード→node A電圧の変換(フルスケール解釈)に局在すると特定し、
...
- **電源**: 受信機の `VCC`/`GND` は PCM1808 と同じ ESP32 の 5V/3.3V レールから分岐する。
  新規の給電経路は要らない
- **UART**: 受信機の `TXD`/`RXD` を ESP32 の空き GPIO(候補: **GPIO13(TX)・GPIO14(RX)**。
  除外リスト — GPIO33〜37(octal PSRAM)・26〜32(flash)・43/44(コンソール)・19/20(USB)・
  0/3/45/46(ストラップ)・15〜18(I2S)・4〜8(LED) — に当たらない未使用ピン。
  当初はGPIO1/2を候補にしていたが、実配線の取り回しの都合でGPIO13/14に変更した
  (2026-08-15。GPIO46は除外リストのストラップ対象かつS3では入力専用なので不可))に接続し、
...
この節の GPIO13/14・部品定数は**設計案であり実配線・実測はしていない。** 現物配線時に
