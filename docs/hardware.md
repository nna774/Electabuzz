...
- **分圧比は PCM1808 データシート(TI, DS 版)のフルスケール入力仕様から逆算した。**
  `ANALOG INPUT` 電気特性: **Input voltage = 0.6 VCC Vp-p**、**Center voltage(VREF)= 0.5 VCC**、
  **Input impedance = 60kΩ(typ.)**。VCC=5V(Recommended Operating Conditions)なら
  **フルスケール = 3.0 Vpp**。公称振幅でフルスケールの70%程度を狙い、サージと
  無負荷電圧上昇のヘッドルームを残す。クリップすると位相推定が壊れるので、余裕側に振るのが正しい
- **R1 = 100kΩ、R2 = 6.8kΩ(どちらも E24 標準品)に確定。** 分圧比 k = R2/(R1+R2) ≒ 0.0637。
  設計上のワーストケース入力 34Vpp に対し分圧後 2.17Vpp(FS比 72%)。
  **ADC の入力インピーダンス 60kΩ(typ.)が R2 と並列に効く**ため、実効分圧比は k ≒ 0.0576 まで下がり、
  同条件で分圧後 1.96Vpp(FS比 65%)。実測の無負荷出力 29.1Vpp では FS比 56〜62% で、
  いずれもクリップから十分な余裕がある。R1 を 100kΩ 側に寄せたのは万一の異常時の電流制限を優先したため
- LPF(C1・TVS・Vcc/2 バイアスの具体的な抵抗/コンデンサ値)と OP アンプの選定は**未確定**。
  Sallen-Key の 2次 LPF は OP アンプが要るため、まず OP アンプを決めてから追って計算する
