...
> **巻数比は暫定値: ≈10.07倍(2026-08-15実測、継続確認中)。** DMM2台で壁側・二次側を
> 同時に実測(102.8V / 10.21V)して算出。直前に同一DMM1台で数秒ずらして測った参考値
> (102.9V / 10.19V → 10.10倍)とも比で0.3%以内に一致しており、この時間スケールでは
> 壁電圧の変動が無視できることも確認できた。旧暫定値 9.71倍(2026-08-03算出、
> 「現物検証ログ」本ファイル135行目のDMM実測10.3VACを100/10.3で近似)からの約4%のズレは、
> 当時壁側電圧を実測せず100V決め打ちで計算していたことに起因する(実際の壁は
> 102.8〜102.9V付近だった)。**複数回・複数日にわたる再測定で安定性を見てから確定値へ
> 格上げする予定**(日本のコンセントは実際には101〜107V程度でばらつくため)。
