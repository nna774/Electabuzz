...
- **分圧比は 1/14.6 を出発点とする。** PCM1808 のフルスケール入力は
  **0.6 × VCC**（データシート値。5V なら 3 Vp-p）。**`VCC` は ESP32 の `5V` ピンから取り、
  その実測は 4.84V**（2026-08-05、テスタ。USB VBUS からショットキー1個ぶん落ちた値）
  なので**フルスケールは 2.904 Vpp**。二次側の実測 29.6 Vpp に対して 70% を狙うと
  `2.033 / 29.6 = 1/14.56`。**例: R1 = 150kΩ / R2 = 11kΩ（1/14.64、フルスケールの 69.6%）。**
  一次側を大きく取れという下の方針とも合う。
  **これは実測 2つ（VCC と二次側）に基づく計算であって、AFE の実測ではない。**
  DMM が届いたら壁電圧と実配線での二次側を測って詰め直す。
  クリップすると位相推定が壊れるので、迷ったら余裕側に振れ
  （R1 = 130k / R2 = 10k だと 1/14 = 72.8% で、余裕が減るぶん少し攻めている）
...
- **手元のモジュールは入力が電解コンデンサで AC 結合されている。**
  モジュール側が `VREF = 0.5 VCC` へバイアスしているなら、**上図の `Vcc/2` バイアス網は要らない。**
  **未確認なので、DMM でモジュール入力の DC 電位を測ってから決める。余計な回路を先に足すな**
