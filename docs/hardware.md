...
> **巻数比は暫定値: ≈9.71倍(未校正)。** 「現物検証ログ」(本ファイル135行目)の
> DMM(HIOKI 3244-60)実測 10.3 VAC(2026-08-03、100V系コンセントで駆動・無負荷)から
> 100/10.3として算出した第一近似。**壁側の電圧をこの時同時に精密測定してはいない**
> (日本のコンセントは実際には101〜107V程度でばらつく)。壁側電圧表示を実際に
> 作る段になるまでは、この暫定値のまま宙に置いてよい——今の`v_rms_mv`自体(二次側基準)
> の記録・検知には影響しない。確定させるにはDMM2台(またはDMM+オシロ)で壁・二次側を
> 同時に当てる再測定が要る。
