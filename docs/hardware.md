...
> **巻数比は暫定値: ≈10.08倍(2026-08-15実測、継続確認中)。** DMM2台で壁側・二次側を
> 同時に実測(102.9V / 10.21V)して算出。直前に同一DMM1台で数秒ずらして測った参考値
> (102.9V / 10.19V → 10.10倍)とも比で0.2%以内に一致しており、この時間スケールでは
...
> 102.9V付近だった)。**複数回・複数日にわたる再測定で安定性を見てから確定値へ
