...
| **GNSS + アンテナ** | ①~~**`UBX-MON-VER` を投げて版を読む。**~~ **済み**（2026-08-12。`FWVER=SPG 3.01`・`PROTVER=18.00`・`GPS;GLO;BDS`+`QZSS`を確認し、本物のu-blox M8世代シリコンと判断できた。SKU内部名`NEO-8N`(実在しない型番)だった懸念は解消。→ [log/2026-08-12-neo-m8n-ucenter-first-connection.md](log/2026-08-12-neo-m8n-ucenter-first-connection.md)）②~~**SMA にアンテナ用のバイアス電圧が出ているか実測**~~ **済み**（2026-08-13。SMA中心ピン-GND間を実測し約3.2V。u-blox典型値の3.3V付近と一致し、給電が設計通り機能していると確認できた。→ [log/2026-08-13-gnss-antenna-first-outdoor-session.md](log/2026-08-13-gnss-antenna-first-outdoor-session.md)）③**数日ログを取り捕捉衛星数と fix 安定性を実測する**（進行中。2026-08-13、屋外(窓越し)で約2時間11分・numSV平均9.4機・HDOP収束後1〜2台を確認した。**残りは屋内(窓ガラス貼り付け)設置との比較、数日分のログ積み上げ**）。これが NEO-M8T（約$100）を買うか否かの判定そのもの。→ [gnss.md](gnss.md)、[log/2026-08-13-gnss-antenna-first-outdoor-session.md](log/2026-08-13-gnss-antenna-first-outdoor-session.md) |
