...
| **NOMINAL区間（起動直後〜NTPロックまで約10分）の扱い** | 現状は`fs`がNTPで規正される(`kMinObs=8`かつ`kMinSpanSeconds=600`)までGoertzel自体を起動せず、起動直後のデータが完全に欠落する。TEと違いGFRQ本体の周波数値は`fs`誤差がそのまま系統誤差として乗るため、`timebase_source=NOMINAL`のタグを付けて記録しても表示封じだけでは救えない（ノイズと違い平均で消えない）。**「生サンプルを保持して事後補正」は容量的に不成立**（600秒ぶんで約115MB、PSRAM8MBでは40秒しか貯まらない）。代わりに**案A**（NOMINALで即記録・即送信しNTPロック後に補正レコードを別送、適用はダウンストリーム側）と**案B**（window単位のGoertzel出力だけ一時保持しロック後に補正してから送信）の2案がある。**どちらも「`fs`誤差が線形なスカラー補正で戻るか」という未検証の前提に乗っている**——次の一手は`tools/gridfreq/goertzel.py`への既知ppm誤差注入検証（机上、実機不要）。→ [log/2026-08-08-nominal-window-open-question.md](log/2026-08-08-nominal-window-open-question.md) |
