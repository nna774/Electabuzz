...
| **バッチ境界のタイムスタンプジャンプの直し方**（2026-08-08発見。→ [risks.md](risks.md)リスク12、[log/2026-08-08-batch-boundary-timestamp-jump.md](log/2026-08-08-batch-boundary-timestamp-jump.md)） | firmware側(バッチ起点を精密な経過時間の積み上げに変える)かAPI側(隣接点のdt許容を締める)か、あるいは両方か。**片方だけで足りるなら軽い方(API側)を先に試すのが筋が良さそうだが、firmware側の構造的な不整合は残るので根本修正ではない**ことは留意 |
