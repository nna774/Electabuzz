...
| 対象 | メモ |
|---|---|
| **`gWindowQueue`満杯discardのDISCONTINUITY未対応を直す**(→ [risks.md](risks.md)リスク14) | 2026-09-01、firmware(`firmware/src/main.cpp`)を読み機序を特定した——Core0の`loop()`が長時間ブロックする(WiFi再接続待ち・SNTP・アップロードのHTTPS POST等)と`gWindowQueue`(容量8)が満杯になり、`pumpI2s()`(line 281-287)が古い`WindowRecord`を黙って捨てる。この経路はI2SオーバーフローのDISCONTINUITYパス(line 220-231)と別物でフラグを立てない(→ [log/2026-09-01-cycles-jump-wifi-reconnect-correlation.md](log/2026-09-01-cycles-jump-wifi-reconnect-correlation.md))。**修正方針は明確**(discard時もdiscontinuity扱いにするだけ、`lambda`側は対応済み)——実装・実機投入はまだ。detectのRoCoFしきい値校正より優先度が高い(誤検知の温床になっているため) |
