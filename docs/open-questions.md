...
| 対象 | メモ |
|---|---|
| **`measurementTask`分割の実機投入・WiFi遮断試験**(→ [risks.md](risks.md)リスク14) | 2026-09-01、firmware(`firmware/src/main.cpp`)を読み、Core0の`loop()`が長時間ブロックする(WiFi再接続待ち・SNTP・アップロードのHTTPS POST等)と`gWindowQueue`(容量8)が満杯になり`pumpI2s()`が古い`WindowRecord`をDISCONTINUITY無しで黙って捨てる機序を特定。Namazu(`../NamazuHaUrokoGaNai`)が2026-08-11に構造的に同じ問題を「吸い出し/送信の2タスク分割」で解決済みと判明し踏襲、新規タスク`measurementTask`(NTP/PPS回帰・`gWindowQueue`吸い出し・バッチ組み立てを一括で担当、`gFs`/`gPps`の書き手を単一に保つ設計)を実装した(→ [log/2026-09-01-cycles-jump-wifi-reconnect-correlation.md](log/2026-09-01-cycles-jump-wifi-reconnect-correlation.md))。`pio run -e record`ビルド確認済み。**残るのは実機投入と、Namazuと同様の意図的なWiFi遮断試験で`gWindowQueue`が実際に溢れなくなったことの確認**。detectのRoCoFしきい値校正より優先度が高い(誤検知の温床になっているため) |
