...
| 対象 | メモ |
|---|---|
| **`measurementTask`分割のWiFi遮断試験**(→ [risks.md](risks.md)リスク14) | 2026-09-01、`gWindowQueue`満杯時の偽スパイク不具合をNamazu(`../NamazuHaUrokoGaNai`)方式(吸い出し/送信の2タスク分割)を踏襲した`measurementTask`(NTP/PPS回帰・`gWindowQueue`吸い出し・バッチ組み立てを一括担当、`gFs`/`gPps`の書き手を単一に保つ設計)で修正、PR #91を`main`へマージしOTAで実機device 1へ配信・稼働確認まで完了した(`fw_version: 3d87157`、session_id 28→29、PPSロックも正常再確立)(→ [log/2026-09-01-cycles-jump-wifi-reconnect-correlation.md](log/2026-09-01-cycles-jump-wifi-reconnect-correlation.md)、[log/2026-09-01-measurement-task-split-ota-deploy.md](log/2026-09-01-measurement-task-split-ota-deploy.md))。**残るのは実際のWiFi断イベントが起きた時に`gWindowQueue`が実際に溢れなくなった(=偽スパイク再発なし)ことの確認のみ**——今回のOTAは正常系の動作確認に留まる。detectのRoCoFしきい値校正より優先度が高い(誤検知の温床になっているため) |
| **`terraform apply`前の`build_lambda.sh`実行忘れ対策** | 2026-08-23、本体worktreeの`terraform/builds/`(gitignore対象)が陳腐化したまま`terraform apply`され、`electabuzz-api`のコードがdetect実装前へ巻き戻る実害が出た(→ [log/2026-08-23-api-stale-build-events-regression.md](log/2026-08-23-api-stale-build-events-regression.md))。`terraform.tfvars`と同種の「worktree間で手元ファイルが伝播しない」構造。`terraform plan`の`source_code_hash`差分を毎回目視確認する運用で足りるか、build→plan→applyを1コマンドにまとめるラッパーを作るべきか未検討 |
...
| **NTPロック時点での帯付きTE表示** | `docs/timebase.md`「TEは保存する、提示は封じる」の表は元々`NTP`区間も±(`tb_residual_ns`×経過時間)の不確かさ帯つきで描いてよい方針だが、v1(→ [log/2026-08-17-te-absolute-display-design.md](log/2026-08-17-te-absolute-display-design.md))では実装しなかった。理由: `PpsTimebase::kMinSpanSeconds`=30秒に対し`NtpTimebase::kMinSpanSeconds`=600秒で、実運用ではPPSロックの方が先に来ることの方が多く、NTP限定の状態が長時間続くケースが稀と見積もったため。**GNSS未接続のまま長期間NTPのみで運用する個体が出た場合、または帯表示自体の需要が具体的に出た場合に着手すれば足りる** |
| **コアダンプ自動収集・アップロード**(なまず(`../NamazuHaUrokoGaNai`)の`CoredumpQueue`ライブラリ、2026-08-29〜09-01実装) | 起動直後(WiFi接続前)に`esp_core_dump_image_get()`でハードウェアcoredump領域の有無を確認しLittleFS`/coredump/`へリングバッファでコピー(コピー確認後に消去)、WiFi接続後・`Uploader`生成前にHMAC署名付きで`ingestUrl + "/coredump"`へ直接POST(`batch-uplink`の`Uploader`は経由しない設計)。**Electabuzzの`default_16MB.csv`には元からcoredump領域(`0xFF0000`・64KB)があり、なまずが必要とした専用パーティションcsvの手作りが要らない分、移植は楽な部類**——`CoredumpQueue`自体は独立ライブラリなのでほぼそのまま持ってこられる(`main.cpp`側の呼び出し位置を`env:s3`/`gridfreqtest`/`record`/`provision`各ビルドモードで意識する程度)。クラウド側も`lambda/ingest`への`POST /coredump`ルート追加・S3キー生成・`terraform/s3.tf`への60日ライフサイクル追加という定型作業で済む(watchdog Lambdaの通知基盤を流用可能)。symbolizeはなまず側もLambda未自動化で手元`esp-coredump`作業のまま。**「実機ではまだクラッシュ事例が無い」という当初の緊急性判断は誤りだったと判明した**——2026-09-02、実データ監査(`series/`のsession_id変化を全期間洗い出し)で2026-08-26 20:14〜20:23 UTCに未記録の再起動(AC電源は正常のまま、ファームウェア側の再起動と推定)を発見した(→ [log/2026-09-02-unrecorded-reboot-2026-08-26-audit.md](log/2026-09-02-unrecorded-reboot-2026-08-26-audit.md))。**優先度は上げてよい候補になった**。ただし**コアダンプ本体より先に、なまずが送っている`reset_reason`ヘッダ(`X-Namz-Reset-Reason`相当、`esp_reset_reason()`を1バイト便乗させるだけ)を追加する方がコストが遥かに低く即効性がある**——WDT/panic/意図しない`ESP.restart()`の切り分けだけなら、パーティション・LittleFS・ingest新ルートを一切要らずに手に入る。コアダンプ機構はその上で「なぜクラッシュしたか」まで掘り下げたくなった時の次段として位置づける → [log/2026-09-02-namazu-coredump-review.md](log/2026-09-02-namazu-coredump-review.md) |
| **`tools/check_pps_soak.py`が8日を超える範囲指定を警告なく打ち切る** | `lambda/s3keys.py`の`series_hour_prefixes()`が`MAX_HOUR_PREFIXES = 24 * 8`(API側の暴走防止用の多重防御定数)を無条件に共有しているため、`--start`/`--end`で8日を超える範囲を指定すると**エラーも警告も出さず**に8日分だけ見て「イベントなし」を返しうる。2026-09-02、17日分の再起動監査(→上記コアダンプ行)でこれを実際に踏み、8日以内のチャンクに分けて手動回避した。**直すなら**: ツール側で範囲が`MAX_HOUR_PREFIXES`を超えたら明示的にエラーにするか、内部で自動チャンク分割する。長期監査を今後もやるなら先に直した方がよい → [log/2026-09-02-unrecorded-reboot-2026-08-26-audit.md](log/2026-09-02-unrecorded-reboot-2026-08-26-audit.md) |
