...
| 対象 | メモ |
|---|---|
| **`gWindowQueue`のdrainをタスク分割する**(→ [risks.md](risks.md)リスク14) | 2026-09-01、firmware(`firmware/src/main.cpp`)を読み、Core0の`loop()`が長時間ブロックする(WiFi再接続待ち・SNTP・アップロードのHTTPS POST等)と`gWindowQueue`(容量8)が満杯になり`pumpI2s()`(line 281-287)が古い`WindowRecord`をDISCONTINUITY無しで黙って捨てる機序を特定。ユーザー指摘でNamazu(`../NamazuHaUrokoGaNai`)が2026-08-11に構造的に同じ問題(`gBatchQueue`のWiFi待ち溢れ)を「吸い出し/送信の2タスク分割」で解決済みと判明し、**それを踏襲する方針に確定した**(データを失わないぶん、discontinuityフラグだけ足す案より優れる)。前提の`Uploader`mutex・RAM満杯時`flushToSpill()`はElectabuzzが現在ピンする`batch-uplink` v3.1.0に既にあるため、firmware側の変更だけで着手できる(→ [log/2026-09-01-cycles-jump-wifi-reconnect-correlation.md](log/2026-09-01-cycles-jump-wifi-reconnect-correlation.md))。**実装・実機投入はまだ**。detectのRoCoFしきい値校正より優先度が高い(誤検知の温床になっているため) |
