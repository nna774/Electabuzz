...
| 対象 | 決めること |
|---|---|
| **`v_rms_mv` の基準点** | **商用の 100V は mV では u16 に収まらない**（最大 65.535V）。トランス二次側の実効値[mV]なら 10.5V ≒ `10500` で収まる。二次側の値を持つか、壁側に換算して単位を変える（10mV 刻み等）か。**分圧比の出発点は 1/14 に決まった**（→ [hardware.md](hardware.md)。フルスケール 3Vpp @ VCC=5V から逆算）が、**DMM での実測待ち**なので確定はしていない。レコードを1バイトも記録していない今なら変更が無料で、記録開始後は高くつく。→ [wire-format.md](wire-format.md) |
