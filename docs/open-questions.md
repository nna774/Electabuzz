...
| **NOMINAL区間（起動直後〜NTPロックまで約10分）の扱い** | 現状は`fs`がNTPで規正される(`kMinObs=8`かつ`kMinSpanSeconds=600`)までGoertzel自体を起動せず、起動直後のデータが完全に欠落する。TEと違いGFRQ本体の周波数値は`fs`誤差がそのまま系統誤差として乗るため、`timebase_source=NOMINAL`のタグを付けて記録しても表示封じだけでは救えない（ノイズと違い平均で消えない）。代案として、生サンプルを保持しNTPロック後に確定した回帰係数(その600秒間全体のフィット)を遡って適用する事後補正案がある——ただしGoertzelの再帰フィルタは`fs`前提でビン周波数をチューニングしているため、単純な線形補正で戻るかは未検証。3択（現状維持／NOMINALタグ付きで記録／生サンプル保持して事後補正）を比較検討する。→ [log/2026-08-08-nominal-window-open-question.md](log/2026-08-08-nominal-window-open-question.md) |
