...
| 対象 | メモ |
|---|---|
| **WiFi断→再接続でcyclesがジャンプする原因のfirmware側調査**(→ [risks.md](risks.md)リスク14) | 2026-09-01、WiFi断・再接続のタイミング(S3アップロードのバースト着弾パターン)と累積位相`cycles`の突発ジャンプ(≒6秒・1秒ぶん、`DISCONTINUITY`未検出)の相関を確認したが、firmware側でなぜ・どこでcyclesが壊れるかは未特定(→ [log/2026-09-01-cycles-jump-wifi-reconnect-correlation.md](log/2026-09-01-cycles-jump-wifi-reconnect-correlation.md))。WiFi再接続イベントハンドラと`GoertzelEstimator`/I2Sのタイミング(ISR・Core分担)の相互作用を疑っている。応急策(WiFi再接続時に`DISCONTINUITY`を立てる)だけ先に入れるか、原因を先に特定するかは未検討。detectのRoCoFしきい値校正より優先度が高い(誤検知の温床になっているため) |
