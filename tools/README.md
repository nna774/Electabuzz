...
- `get_object` だけラップして「ローカルにあれば読む・無ければ取ってバイト列を
  そのまま保存する」薄いキャッシュクライアントを渡せばよい。**この薄いラッパーは
  `tools/s3cache.py`（`s3cache.cached_client()`）として共通化してある。新しく
  `series/` を読むスクリプトを書く時は、都度書き直さずこれを使うこと。**
- worktreeで作業していても `.s3cache/` は**メインチェックアウト側の絶対パス**を
  指すようにする（worktreeごとに毎回別ディレクトリだとキャッシュが効かない）。
  `tools/s3cache.py` はこれを内部で解決済み。
