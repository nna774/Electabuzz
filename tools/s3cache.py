"""S3の`series/`をローカルにキャッシュする`get_object`ラッパー。

`tools/README.md`「S3の`series/`を取ってくる時のキャッシュ」の方針を、都度
書き直さずに使えるよう1本化したもの。閾値や集計窓を変えながら同じ区間を何度も
読み直す解析（`check_pps_soak.py`等）で使う。
[NamazuHaUrokoGaNai](https://github.com/nna774/NamazuHaUrokoGaNai)の
`tools/s3cache.py`と同じ設計。

- **object keyをそのまま`.s3cache/`以下にミラーする**（時刻範囲で丸ごと切った窓単位で
  キャッシュすると、窓をわずかにずらしただけで丸ごと引き直しになるため。バッチ
  (30秒粒度)単位でキャッシュすれば、窓が重なっている限り差分だけ取得すれば済む）。
- **`list_objects_v2`は常に本物のS3へ通す**（新着バッチを見逃さないため。コストも
  ほぼ無い）。キャッシュするのは`get_object`だけ。`series/`は書き込み後不変なので安全。
...
class CachedS3:
    ...
    `store_gridfreq.list_series_keys_in_range`等、`list_objects_v2`/`get_object`しか
    使わない箇所にそのまま渡せる。
    """
...
def cached_client(s3=None) -> CachedS3:
    """使い方: `s3 = s3cache.cached_client()` を`store_gridfreq`等にそのまま渡す。"""
    return CachedS3(s3)
