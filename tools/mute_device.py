#!/usr/bin/env python3
"""デバイスをwatchdogの欠測監視から外す/戻す手元用CLI（Namazuの
`tools/mute_device.py`からの移植）。

退役デバイスや、ハード試験のたびに繋いでは黙る試験機向け。mute中はwatchdogが
完全に無視するので再送スパムが止まる。ingestが実際にバッチを受信すると自動で
unmuteされる（`lambda/common/watchdog_mute.py`参照）ので、次に試験を始める時に
手動でunmuteし直す必要は無い——試験が終わって黙ったタイミングで、このCLIで
再度muteするだけでよい。

使い方（AWS認証情報とリージョンは通常のboto3の解決に従う）:

    export NAMZ_DEVICES_TABLE=electabuzz-devices   # or pass --table

    python tools/mute_device.py mute 1          # 監視対象外にする
    python tools/mute_device.py mute 1 --yes    # 確認プロンプトを省略
    python tools/mute_device.py unmute 1        # 監視対象に戻す（手動）
    python tools/mute_device.py list            # 現在muteされているデバイスを一覧
"""
...
# Namazuのように --id ごとの払い出しテーブル解決は要らない。→ tools/request_ota.py)。
